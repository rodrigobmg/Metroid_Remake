...
SDL_Surface* scaleSurface(const SDL_Surface* const surf, int width, int height) {
  ...
  SDL_BlitScaled(const_cast<SDL_Surface*>(surf),nullptr,sub,&rect);
...
int PerPixelCollisionStrategy::execute2(
      const Drawable& obj1, const Drawable& obj2) const {
  RectangularCollisionStrategy strategy;
  if ( not strategy.execute(obj1, obj2) ) return false;
  // If we got this far, we know that the sprite rectangles intersect!
  ...
  for (int x = sides[1]; x < sides[2]; ++x) {
    for (int y = lids[1]; y < lids[2]; ++y) {
      ...
      if ( isVisible(pixel1,surface1) && isVisible(pixel2,surface2) ) {
        ...
        if (i < .1 * width1 && j > .9 * height1) return 1;
        else if (i > .9 * width1 && j > .9 * height1) return 2;
        else if (j > .9 * height1) return 3;
        else return 4;

        /*if (j > .9 * height1) {
          if (i < .1 * width1) return 1;
          else if (i > .9 * width1) return 2;
          else return 3;
        }
        else if (j < .1 * height1) {
          if (i < .9 * width1) return 4;
          else if (i > .9 * width1) return 5;
          else return 6;
        }
        else {
          if (i < .1 * width1) return 7;
          else if (i > .9 * width1) return 8;
          else return 9;
        }*/
      }
    }
